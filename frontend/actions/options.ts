...
const DjangoAdapter = (): Adapter => {
  return {
    async createVerificationToken(verificationToken: VerificationToken) {
      return await post(`${API_URL}/api/auth/verification`, verificationToken);
    },
    ...
    async createUser(user: Omit<AdapterUser, "id">): Promise<AdapterUser> {
      const createdUser = await post<AdapterUser>(`${API_URL}/api/auth/user`, user);
      if (!createdUser) {
    ...
    async getUser(id) {
      return await get(`${API_URL}/api/auth/user`, { id });
    },

    async getUserByEmail(email) {
      return await get(`${API_URL}/api/auth/user`, { email });
    },

    async getUserByAccount({ providerAccountId, provider }) {
      return await get(`${API_URL}/api/auth/user`, { providerAccountId, provider });
    },
    ...
    async linkAccount(account: AdapterAccount): Promise<AdapterAccount> {
      const linkedAccount = await post<AdapterAccount>(`${API_URL}/api/auth/account`, account);
      if (!linkedAccount) {
    ...
    async createSession({ sessionToken, userId, expires }): Promise<AdapterSession> {
      const session = await post<AdapterSession>(`${API_URL}/api/auth/session`, { sessionToken, userId, expires });
      if (!session) {
    ...
    async deleteSession(sessionToken) {
      await post(`${API_URL}/api/auth/session`, { sessionToken });
    },

    async unlinkAccount(partialAccount) {
      await post(`${API_URL}/api/auth/account`, partialAccount);
    },
  };
};
