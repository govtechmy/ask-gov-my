...
const toast_viewport_cva = cva([
  "fixed bottom-0 flex max-h-screen w-full overflow-hidden flex-col-reverse gap-1",
  "p-4 sm:right-0 sm:top-auto sm:flex-col md:max-w-sm",
]);
...
const toast_cva = cva([
  "group pointer-events-auto relative bg-white-focuswhite200 z-[99] p-3 flex w-full items-start gap-2 outline-none",
  "overflow-hidden rounded-md border shadow-card border-washed-100 transition-all data-[swipe=cancel]:translate-x-0 ",
...
const toast_close_cva = cva([
  "hover:text-black-700 absolute right-1 top-1 rounded-md p-1",
  "focus:outline-none",
]);
...
>(({ className, ...props }, ref) => {
  return (
    <ToastPrimitives.Description
      ref={ref}
      className={cn("text-black-700 text-body-sm", className)}
      {...props}
    />
  );
});
...
const toast_progress_cva = cva("w-full h-1", {
  variants: {
    variant: {
      ...
      info: "bg-askmygovbrand-700",
      warning: "bg-warning-700",
      error: "bg-danger-700",
      message: "bg-washed-100",
    },
  },
});
